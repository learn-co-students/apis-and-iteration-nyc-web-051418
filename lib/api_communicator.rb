...
def get_character_movies_from_api(search_value)
  ...
  films_array = find_target(search_value)
...
def show_movie_information(search_value)
  ...
  # end
  found_data_array = find_target(search_value)
...
def find_target(search_value)
  search_value["search_type"] == "films" ? key_type = "title" : key_type = "name"
  movie_results_array = get_info_from_api(search_value)
  found_data_array = movie_results_array.find do |movie_data_package|
    movie_data_package[key_type].downcase == search_value["name"]
  end
